...
//				myControl.util.dump("BEGIN myControl.ext.mob_customizer.calls.cartItemsAdd.dispatch");
...
//for a list of keys/values, see
			onSuccess : function(P)	{
				...
				myControl.ext.mob_customizer.actions.initConfigurator(P);
...
//simple enough.  used when page is printed. set printme=1 on URI.
		printPage : {
			onSuccess : function(tagObj)	{
				var containerPID = myControl.ext.mob_customizer.vars.uriParams.s2;
...
//displays the list of subcategories for .drawers (step 3). gets applied to call during app init.
		displayDrawers : {
			onSuccess : function(tagObj)	{
//				myControl.util.dump('BEGIN mob_customizer.callbacks.displayDrawers.onSuccess ');
...
//gets executed (eventually) once a category is selected in step 1. shows the product for that category.
		displayStorageContainers : {
			onSuccess : function(tagObj)	{
//				myControl.util.dump('BEGIN myControl.ext.mob_customizer.callbacks.displayStorageContainers.onSuccess ');
//				myControl.util.dump(' -> datapointer = '+tagObj.datapointer);
//could use tagObj.datapointer.split('|')[0] instead of hard coding IF we need that flexibility. if not, just hard code it so no extra work has to be done (faster).
...
// will populate product in spot 2.
		containerCatSelected : {
			...
			onSuccess : function(tagObj)	{
				myControl.ext.mob_customizer.util.containerCatSelected(tagObj.datapointer.split('|')[1]);
				},
			onError : function(responseData,uuid)	{
				myControl.util.handleErrors(responseData,uuid)
				}
...
// will display preview in middle column and adjust number of 'spots' in selected drawers.
		containerSizeSelected : {
			onSuccess : function(tagObj)	{
				myControl.ext.mob_customizer.util.containerSizeSelected(tagObj.datapointer.split('|')[1]);
				},
			onError : function(responseData,uuid)	{
				myControl.util.handleErrors(responseData,uuid)
				}
...
//will open one of the drawer categories and load product data.
		...
		drawerCatSelected : {
			onSuccess : function(tagObj)	{
				myControl.ext.mob_customizer.util.drawerCatSelected(tagObj.datapointer.split('|')[1]);
				},
			onError : function(responseData,uuid)	{
				myControl.util.handleErrors(responseData,uuid)
				}
...
//will apply a draw to a 'spot', incuding necessary 'selected drawer' info in right col.
		addDrawerToSpot :{
			onSuccess : function(tagObj)	{
				myControl.ext.mob_customizer.util.drawerAssignedToSpot(tagObj.spot,tagObj.datapointer.split('|')[1])
				},
			onError : function(responseData,uuid)	{
				myControl.util.handleErrors(responseData,uuid)
				}
...
//			myControl.util.dump('BEGIN mob_customizer.actions.initConfigurator');
...
// ### SANITY - url 'may' have been nuked by pop/push state by now. but uri vars are handled through handleAppInit
			if(!$.isEmptyObject(P))	{
				myControl.ext.mob_customizer.vars.uriParams = $.extend(myControl.ext.mob_customizer.vars.uriParams,P);
//				myControl.util.dump(" -> use selections passed into function.");
//				myControl.util.dump(myControl.ext.mob_customizer.vars.uriParams);
				}
			else if(!$.isEmptyObject(myControl.ext.mob_customizer.vars.uriParams))	{
...
//				myControl.util.dump(" -> there are selections in localStorage (from a previous visit).");
				myControl.ext.mob_customizer.vars.uriParams = myControl.storageFunctions.readLocal('configurator|uriParams');
//				myControl.util.dump(myControl.ext.mob_customizer.vars.uriParams);
				}
			else	{
				myControl.ext.mob_customizer.vars.uriParams = myControl.util.getParametersAsObject(document.location.href.split('?')[1]);
//					myControl.util.dump(" -> if selections are on URI, use them.");
				}

			myControl.ext.mob_customizer.vars.uriParams.printMe = null; //never set printme in uriParams. could cause accidental execution of print code.

//if certain vars are not set, apply some defaults.
			if(!myControl.util.isSet(myControl.ext.mob_customizer.vars.uriParams.s1))	{
				myControl.ext.mob_customizer.vars.uriParams.s1 = ".storage-containers.major-league-baseball";
//					myControl.util.dump(" -> no storage organizer category set. use default.");
				}
			if(!myControl.util.isSet(myControl.ext.mob_customizer.vars.uriParams.s2))	{
				myControl.ext.mob_customizer.vars.uriParams.s2 = "50092MLB";
//					myControl.util.dump(" -> no storage organizer product set. use default.");
				}
			if(!myControl.util.isSet(myControl.ext.mob_customizer.vars.uriParams.s3))	{
				myControl.ext.mob_customizer.vars.uriParams.s3 = ".drawers.major-league-baseball";
...
//get the details on the open drawer category. saves an extra request. also solves a transmogrify sequencing issue if it's added 2 the Q first.
			numRequests += myControl.ext.store_navcats.calls.appCategoryDetailMax.init(myControl.ext.mob_customizer.vars.uriParams.s3);
...
//there's only 3 bins right now. we're inevitably going to need this data. get it now when a request is most likely happening anyway.
			...
			numRequests += myControl.ext.store_navcats.calls.appCategoryDetailMax.init('.storage-containers',{"callback":"displayStorageContainers","extension":"mob_customizer"});
			numRequests += myControl.ext.store_navcats.calls.appCategoryDetailMax.init('.drawers',{"callback":"displayDrawers","extension":"mob_customizer"});
			
			
			numRequests += myControl.ext.mob_customizer.util.popCustomerFromPresets();
...
//guts of this found here: http://www.dynamicdrive.com/dynamicindex9/addbook.htm
		bookmarkThis : function()	{
			...
			var url = myControl.ext.mob_customizer.util.makeURL();
...
//since in a product list, the product data 'may' not be in memory yet, and else is present if data not available.
			linkToConfigurator : function($tag,data)	{
				if(myControl.data['appProductGet|'+data.value])	{
					...
					var pid = data.value;
					if(pData['zoovy:prod_dimensions'] && pData['user:prod_organization'] && myControl.ext.mob_customizer.util.guessContainerParentCat(pid))	{
						$tag.addClass('pointer').click(function(){
							myControl.ext.mob_customizer.actions.initConfigurator({'s1':myControl.ext.mob_customizer.util.guessContainerParentCat(pid),'s2':pid}); return false;
				...
				if(!$.isEmptyObject(myControl.data.cartItemsList.cart.stuff))	{
					for(index in myControl.data.cartItemsList.cart.stuff)	{
						if(myControl.data.cartItemsList.cart.stuff[index].full_product['zoovy:prod_dimensions']){
							...
							else if(myControl.data.cartItemsList.cart.stuff[index].full_product['zoovy:prod_dimensions'] == '3x1')	{
								numSpots = 3;
								}
							else	{
								//unknown dimensions.
						...
						if(myControl.data.cartItemsList.cart.stuff[index].full_product['is:user1'])	{
							numDrawers += myControl.util.isSet(myControl.data.cartItemsList.cart.stuff[index].qty) ? (myControl.data.cartItemsList.cart.stuff[index].qty * 1): 1;
							numDrawers += 1;
							}
						}
...
//				myControl.util.dump("BEGIN myRIA.renderFormats.view360inModal");
//				myControl.util.dump(" -> data.value: "+data.value);
				$tag.removeClass('displayNone').addClass('pointer');
				$tag.click(function(){
//					myControl.util.dump("viewer clicked. add event here. no. add it in the sho360viewer action.");
					myControl.ext.mob_customizer.actions.show360Viewer(data.value);
					});
...
//				myControl.util.dump("BEGIN mob_customizer.util.containerCatSelected");
				var numRequests = 0;
...
//the container size code needs to happen after the product list is built, otherwise 'classes' assigned are overwritten w/ transmogrify.
				...
				myControl.ext.mob_customizer.util.hideChooser(); //closes an open chooser. feels natural when using to have this happen.
...
//				myControl.util.dump(myControl.ext.mob_customizer.vars.uriParams);
				...
				if(numRequests = myControl.ext.store_prodlist.util.buildProductList({"templateID":"mobStorageContainerProductSpec","parentID":"storageContainerProdlist","csv":myControl.data['appCategoryDetail|'+catSafeID]['@products']})){
					myControl.calls.ping.init({"callback":"containerSizeSelected","extension":"mob_customizer","datapointer":"appProductGet|"+myControl.ext.mob_customizer.vars.uriParams.s2});
					myControl.model.dispatchThis();
					}
				else	{
					//no dispatch is occuring because all data is in memory. execute the code handled in the ping above.
					if(myControl.ext.store_product.calls.appProductGet.init(myControl.ext.mob_customizer.vars.uriParams.s2,{'callback':'containerSizeSelected','extension':'mob_customizer'}))	{myControl.model.dispatchThis()}
					}


//				myControl.util.dump(" -> END mob_customizer.util.containerCatSelected. uriParams to follow: ");
//				myControl.util.dump(myControl.ext.mob_customizer.vars.uriParams);
...
//				myControl.util.dump(myControl.ext.mob_customizer.vars.uriParams);
				...
				myControl.ext.mob_customizer.vars.uriParams.s2 = pid;
...
//					myControl.util.dump("Warning! -> product "+pid+" may not have a size set");
					...
					var spots = myControl.ext.mob_customizer.util.getSpotCountFromDimensions(size); // the # of spots available based on dimensions.
...
//otherwise, they 'stay' in the totals and product list, et all.
							myControl.ext.mob_customizer.util.drawerAssignedToSpot(i);
...
// added 2012-06-15
//							myControl.ext.mob_customizer.util.drawerAssignedToSpot(i,myControl.ext.mob_customizer.vars.uriParams["s3d"+i]);
...
setTimeout("myControl.ext.mob_customizer.util.makeDrawersDraggable()",3500);
				},
...
//				myControl.util.dump("ID for positioning = "+parentID);
				myControl.ext.mob_customizer.util.disableDraggingOnSpots();
//add action on the spots so that, when clicked, they load the appropriate sku into the appropriate location in both the chooser and the preview.
				...
				//		myControl.util.dump(" -> drawer clicked. index +1 = "+(index+1)+" and pid = "+pid);
						...
						myControl.ext.mob_customizer.util.drawerAssignedToSpot(index+1,pid);
						myControl.ext.mob_customizer.util.unbindSpotHotspots();
						});
					});
			...
			unbindSpotHotspots : function()	{
				myControl.ext.mob_customizer.util.enableDraggingOnSpots();
				$('#selectedDrawerContainer li').each(function(index){
...
//				myControl.util.dump("BEGIN mob_customizer.util.drawerAssignedToSpot.");
//				myControl.util.dump(" -> spot = "+spot);
//				myControl.util.dump(" -> pid = "+pid);
				myControl.ext.mob_customizer.vars.uriParams["s3d"+spot] = pid; //save to var object so that items populate when returning to page.
...
//					myControl.ext.mob_customizer.util.bindMouseover2Spot(spot); 
...
//after emptying, put the spot # back in.
				...
				myControl.ext.mob_customizer.util.addToSelectedUL(spot,pid);
				myControl.ext.mob_customizer.util.updateTotals();
...
//				myControl.ext.mob_customizer.vars.uriParams =  myControl.util.getParametersAsObject(myControl.ext.mob_customizer.util.buildURIVars());
...
//					myControl.util.dump("addToAllSpots i = "+i);
					myControl.ext.mob_customizer.util.drawerAssignedToSpot(i,pid);
					}
					myControl.ext.mob_customizer.util.hideChooser();
				}, //addToAllSpots
			...
			bindMouseover2Spot : function(spot)	{
				$('#drawerLoc_'+spot).mouseover(function(){
					...
					$spot.append($("<button>").text('X').addClass('removeButton').click(function(){
						...
						myControl.ext.mob_customizer.util.drawerAssignedToSpot($spot.index()+1,''); //pass blank to remove the item
...
//inArray will return a -1 if false, then return the indexof, which potentially could be a 0
				if($.inArray(pid,myControl.ext.mob_customizer.vars.recentlyRemoved) >= 0)	{
//					myControl.util.dump(" item "+pid+" is already in recentlyRemoved");
					}
				else if(pid != '')	{
//					myControl.util.dump("item "+pid+" being added to recentlyRemoved");
					myControl.ext.mob_customizer.vars.recentlyRemoved.push(pid);
					}
				if(myControl.ext.mob_customizer.vars.recentlyRemoved.length > 0)	{
//once recentlyViewed is populated, it gets displayed and rerendered with each change, so that the carousel updates.
					myControl.ext.store_prodlist.util.buildProductList({"templateID":"mobRecentViewedProductSpec","parentID":"recentlyRemovedProdlist","csv":myControl.ext.mob_customizer.vars.recentlyRemoved,"items_per_page":200})
...
//					$('#recentlyRemovedContainer').show(); //recentlyRemoved starts off hidden (because it's empty). make sure it's visible once populated.
					...
					if(myControl.ext.mob_customizer.vars.recentlyRemoved.length > 1)	{
...
//				myControl.util.dump("BEGIN customizer.util.selectedULTemplate ["+spot+"]");
				if($.isEmptyObject(P) || !myControl.util.isSet(P.pid))	{
					P.name = 'Empty';
				...
				var o = "<div class='floatRight'><span class='number '>"+spot+"<\/span><button  onClick=\"myControl.ext.mob_customizer.util.drawerAssignedToSpot('"+spot+"'); myControl.ext.mob_customizer.util.hideChooser();\"";
				if(!P.pid)
					o += " disabled='disabled' "
				o += ">X<\/button><\/div><div onClick=\"myControl.ext.mob_customizer.util.drawerClicked('"+P.pid+"','selectedDrawerLoc_"+spot+"')\">";
				o += myControl.util.makeImage({"h":"35","w":"35","b":"tttttt","name":P.image,"tag":1})
...
//updates local storage. used if the user leaves the website and comes back.
				myControl.storageFunctions.writeLocal('configurator|uriParams',myControl.ext.mob_customizer.vars.uriParams);
...
//				myControl.util.dump("BEGIN mob_customizer.util.addItAllToTheCart");
...
//add it to the obj if pid isn't already in there, otherwise increment existing value. same drawer may be in several spots.
				...
				if(numDrawers < 2)	{
					...
					myControl.ext.mob_customizer.calls.cartItemsAdd.init({"product_id":$('#storageContainerProdlist .selected').attr('data-pid'),"quantity":1});
					for(index in obj)	{
						myControl.ext.mob_customizer.calls.cartItemsAdd.init({"product_id":index,"quantity":obj[index]});
						}
					...
					$('#modalCart').append(myControl.renderFunctions.createTemplateInstance('cartTemplate',"modalCartContents")); //shows loading. preps for callback.
...
//				myControl.util.dump("BEGIN mob_customizer.util.makeURl");
				var temp;
...
//				adviceWin = window.open(myControl.ext.mob_customizer.util.makeURL('&printme=1&SKIPPUSHSTATE=1'),'advice','status=no,width=600,height=600,menubar=no,scrollbars=yes');
...
//				myControl.util.dump("BEGIN mob_customizer.util.popCustomerFromPresets");
				var numRequests = 0; //the number of requests that will need to be made. returned.
				var P = myControl.ext.mob_customizer.vars.uriParams; //shortcut.
//				myControl.util.dump(P);
...
//					myControl.util.dump(" -> s1 is populated ["+P.s1+"]");
					numRequests += myControl.ext.store_navcats.calls.appCategoryDetailMax.init(P.s1,{"callback":"containerCatSelected","extension":"mob_customizer"});
					}
...
//gets product data each bin specified
				var i;
				for(i = 1; i <= 9; i +=1)	{
//					myControl.util.dump("['s3d"+i+"'] = "+myControl.ext.mob_customizer.vars.uriParams['s3d'+i]);
					if(myControl.util.isSet(myControl.ext.mob_customizer.vars.uriParams['s3d'+i]))	{
//						myControl.util.dump(" -> spot "+i+": "+myControl.ext.mob_customizer.vars.uriParams['s3d'+i]);
						numRequests += myControl.ext.store_product.calls.appProductGet.init(myControl.ext.mob_customizer.vars.uriParams['s3d'+i],{"callback":"addDrawerToSpot","extension":"mob_customizer","spot":i})
						}
					else	{
...
//					myControl.util.dump(" -> s3 is populated ["+P.s3+"]");
					numRequests += myControl.ext.store_navcats.calls.appCategoryDetailMax.init(P.s3,{"callback":"drawerCatSelected","extension":"mob_customizer"}); 
					}
				return numRequests;
...
//		myControl.util.dump(" -> new index : "+$(this).index());
		myControl.ext.mob_customizer.util.drawerAssignedToSpot($(this).index()+1,ui.draggable.attr('data-pid'))
		if(ui.draggable.attr('data-formerparentindex'))	{
			myControl.ext.mob_customizer.util.drawerAssignedToSpot((ui.draggable.attr('data-formerparentindex') *1)+1);
			}
