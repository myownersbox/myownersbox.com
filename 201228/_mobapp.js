...
/*
The changes to quickstart include support for aitrk uri param being saved onLoad and also passed when going to checkout.
some updates were made to support buyer address change. This are supported changes as of V-201235
*/
...
var aitrk = myControl.util.getParameterByName('aitrk');
if(aitrk)	{
	myControl.ext.myRIA.vars.aitrk = aitrk;
	if(document.location.protocol == 'https:')	{
		myControl.storageFunctions.writeCookie('aitrk',aitrk);
		}
	}
...
//							$buyerAddresses.append(type == '@bill' ? '<h2>Billing Address(es)</h2>' : '<h2>Shipping Address(es)</h2>');
							...
							for(var i = 0; i < L; i += 1)	{
								$buyerAddresses.append(myControl.renderFunctions.transmogrify({
									'id':addressClass+'_address_'+myControl.data.buyerAddressList[type][i]['_id'],
...
//									myControl.util.dump(" -> cmdObj: "); myControl.util.dump(cmdObj);
									...
									myControl.ext.myRIA.calls.buyerAddressAddUpdate.init(cmdObj, {'callback':'handleBuyerAddressUpdate','extension':'myRIA','parentID':$parent.attr('id')},'immutable');
	...
	//					myControl.util.dump(" -> Got to search case.");	
					...
					case 'customer':
						if('file:' == document.location.protocol || 'https:' == document.location.protocol)	{
							...
							if(myControl.ext.myRIA.vars.aitrk)	{
								SSLlocation += "&aitrk="+myControl.ext.myRIA.vars.aitrk
								}
...
// if we redirect to ssl for checkout, it's a new url and a pushstate isn't needed, so a param is added to the url.
							...
							var SSLlocation = myControl.vars.secureURL+"#checkout?show=checkout&sessionId="+myControl.sessionId;
							if(myControl.ext.myRIA.vars.aitrk)	{
								SSLlocation += "&aitrk="+myControl.ext.myRIA.vars.aitrk
								}
							document.location = SSLlocation;
...
//				myControl.util.dump("P.uriParams follows:"); myControl.util.dump(P.uriParams);
				if(P.uriParams.meta)	{
					myControl.calls.cartSet.init({'meta':P.uriParams.meta},{},'passive');
					myControl.model.dispatchThis('passive');
					}
...
//							myControl.util.dump(" -> myaccount article loaded. now show addresses...");
				...
				myControl.ext.myRIA.util.handleTemplateFunctions(P);
				$('#mainContentArea_customer, #mainContentArea').removeClass('loadingBG');
				},  //showCustomer
...
//myControl.util.dump("BEGIN myRIA.util.showPage("+P.navcat+")"); myControl.util.dump(P);
...
if(!catSafeID)	{
	...
	var tagObj = $.extend(P,{'callback':'fetchPageContent','extension':'myRIA','templateID':P.templateID,'parentID':parentID});
	
	myControl.ext.store_navcats.calls.appCategoryDetailMax.init(catSafeID,tagObj);
	myControl.model.dispatchThis();
	}
