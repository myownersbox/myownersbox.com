...
//					myControl.util.dump(' -> multi page product list.');
					...
					if(!myControl.ext.store_prodlist.vars[parentID].hide_multipage)	{
						$parent.before(myControl.ext.store_prodlist.util.showMPControls(parentID));
						}
					}
...
myControl.util.dump("BEGIN store_prodlist.util.showMPControls [parentID: "+parentID+"]");

				$('#mpControl_productListContainer .mpControls').empty().remove(); 
