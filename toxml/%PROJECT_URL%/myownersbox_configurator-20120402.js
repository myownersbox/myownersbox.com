...
//could use tagObj.datapointer.split('|')[0] instead of hard coding IF we need that flexibility. if not, just hard code it so no extra work has to be done (faster).
				if(myControl.ext.store_navcats.util.getChildDataOf('.drawers',{'parentID':'drawerCategories','callback':'addCatToDom','templateID':'mobDrawerChooser','extension':'store_navcats'},'appCategoryDetail')){myControl.model.dispatchThis()}
...
//					myControl.util.dump(" -> spot "+($(this).index()+1)+" data-pid: "+$(this).attr('data-pid'));
					if($(this).attr('data-pid')) {total += (myControl.data['appProductGet|'+$(this).attr('data-pid')]['%attribs']['zoovy:base_price'] * 1)}
					});
				if(total)	{
...
//if no pid is passed, just empty the existing spots
				if(pid)	{
					var $dragContainer = $("<span class='draggable'>").attr('data-pid',pid).html(myControl.util.makeImage({"h":"129","w":"129","bg":"ffffff","name":myControl.data['appProductGet|'+pid]['%attribs']['zoovy:prod_image1'],"tag":1}));
