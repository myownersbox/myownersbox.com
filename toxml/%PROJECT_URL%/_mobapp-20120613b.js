...
//successful login.	
				...
				$('#loginSuccessContainer').prepend("Thank you, you are now logged in.");
				$('#modalLoginForm').hide();
...
//handle cases where the homepage is treated like a category page. happens in breadcrumb.
				...
				if(P.isInit)	{
					relativePath = ''; //on init, don't mess w/ URL but add a popState. That will allow the initial page to 'reload' on back, but not add any 'history'
					}
//				$('title').text("MyOwnersBox.com"+relativePath);
				document.title = "MyOwnersBox.com"+relativePath;
				try	{
...
//make sure form is showing and previous messaging is removed/reset.
				...
				$('#modalLoginForm').show(); //contains actual form.
...
//make sure they're available.
			showArticle : function(subject,P)	{
				myControl.util.dump("BEGIN myRIA.util.showArticle 1.1 ("+subject+")");
				myControl.util.dump(" -> P:");
				myContorl.util.dump(P);
...
//if P.orderID is set, there's a chance we just got redirected from checkout. It may be a refresh tho, so look if the order is in localStorage.
// it'll only be there on the computer it was just placed from. Fetch it. Check the timestamp to make sure it's recent.
				if(P && P.orderid)	{
					myControl.util.dump(" -> P and P.orderid ["+P.orderid+"] are set");
					var dp = 'order|'+P.orderid;
					myControl.model.fetchData(dp);
					if(!$.isEmptyObject(myControl.data[dp])	{
						var age = myControl.util.unixNow - myControl.data[dp].ts;
						if(age > (60*60*4))	{
							myControl.data[dp] = {}; //too much time has passed between order completion and now. don't show.
							}
						}
					}
					
				if(subject == 'orders' && typeof P == 'object' && P.orderid && !$.isEmptyObject(myControl.data['order|'+P.orderid]) && )	{
					myControl.util.dump(" -> most likely this was a link from the order complete page to view the invoice.");
					$('mainColumn').append(myControl.renderFunctions.transmogrify({'id':'order_'+P.orderid,'orderid':P.orderid},'orderContentsTemplate',myControl.data['order|'+P.orderid]));
					}
