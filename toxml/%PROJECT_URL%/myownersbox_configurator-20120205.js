...
//				myControl.util.dump('BEGIN myControl.ext.store_navcats.init.onSuccess ');
		...
		initConfigurator : {
			onSuccess : function()	{
//				myControl.util.dump('BEGIN myControl.ext.myownersbox_configurator.callbacks.initConfigurator.onSuccess');
...
//could use tagObj.datapointer.split('|')[0] instead of hard coding IF we need that flexibility. if not, just hard code it so no extra work has to be done (faster).
		...
		itemAddedToCart :	{
			onSuccess : function(tagObj)	{
				...
				$('#addToCartBtn').removeAttr('disabled').before(myControl.util.formatMessage({'message':'Item(s) added to the cart!','uiIcon':'check'}))
				
				},
			onError : function(d)	{
				myControl.util.dump('BEGIN myControl.ext.store_product.callbacks.init.onError');
				$('#addToCartBtn').removeAttr('disabled').before(myControl.util.getResponseErrors(d))
				}
...
//the css hover menu doesn't close on click. This is a workaround. hides the dropdown, then turns it back on after two seconds.
				...
				if(numRequests = myControl.ext.store_prodlist.util.buildProductList({"templateID":"mobStorageContainerProductSpec","parentID":"storageContainerProdlist"},myControl.data['categoryDetail|'+catSafeID]['@products'])){
					myControl.model.dispatchThis();
					}
...
//				myControl.util.dump("safeid = "+catSafeID);
				...
				if(myControl.ext.store_prodlist.util.buildProductList({"templateID":"mobDrawerProductSpec","parentID":"mobDrawerChooser_"+myControl.util.makeSafeHTMLId(catSafeID)},myControl.data['categoryDetail|'+catSafeID]['@products']))
					myControl.model.dispatchThis();
				},
...
//add action to 'add to all' button so that it changes every 'spot' to the focus sku if clicked.
				$('#drawerChooserAddToAllBtn').click(function(){
...
//once recentlyViewed is populated, it gets displayed and rerendered with each change, so that the carousel updates.
					myControl.ext.store_prodlist.util.buildProductList({"templateID":"mobRecentViewedProductSpec","parentID":"recentlyRemovedProdlist"},myControl.ext.myownersbox_configurator.vars.recentlyRemoved)
...
//!!! not quite done yet.  Needs a call (in this ext) that lets the cmdObj be passed in.
/// !!! needs validation
//attach a callback to this that shows all good or something bad.
			addItAllToTheCart : function()	{
				...
				var storageSku = $('#storageContainerProdlist .selected').attr('data-pid');
				myControl.util.dump(" -> storage sku: "+storageSku);
...
//<input type="hidden" value="1" name="product_id:MODEL08">
//<input type="text" style="text-align:center;" size="2" value="0" name="quantity:MODEL08" class="zform_textbox">


				for(index in obj)	{
					cmdObj['product_id:'+index] = 1;
					cmdObj['quantity:'+index] = obj[index];
					}
				cmdObj['product_id:'+storageSku] = 1;
				cmdObj['quantity:'+storageSku] = 1;
				
				myControl.ext.myownersbox_configurator.calls.addToCart.init(cmdObj);
...
*/
			makeURL : function()	{
				...
				var url = "?"; //the url. what is returned.
				url += "s1="+s1;
