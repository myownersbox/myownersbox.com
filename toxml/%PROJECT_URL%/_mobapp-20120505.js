...
//go get the root level categories, then show them using showCategories callback.
				...
				myControl.ext.myRIA.util.handleLegacyURL(); //checks url and will load appropriate page content.
...
//list is empty.
					...
					myControl.ext.store_prodlist.util.buildProductList({"templateID":"productListTemplate","parentID":tagObj.parentID,"csv":prods})
					myControl.model.dispatchThis();
					}
				},
			onError : function(responseData,uuid)	{
...
//returns # of requests needed. so if 0 is returned, no need to dispatch.
					myControl.ext.store_prodlist.util.buildProductList({"templateID":"productListTemplate","parentID":"resultsProductListContainer","items_per_page":20,"csv":myControl.data[tagObj.datapointer]['@products']})
					}
...
//that parent ID is prepended to the sku and used in the list item id to decrease likelyhood of duplicate id's
	...
	//creates an object in myControl.ext.store_prodlist.vars such as items per page, # pages, etc.
					myControl.ext.store_prodlist.util.setProdlistVars({'parentID':parentListID,'csv':data.value,'items_per_page':15,'templateID':data.bindData.loadsTemplate})
...
//rather than having all the params in the dom, just call this function. makes updating easier too.
	...
	//nuke existing content and error messages.
					...
					$('#mainContentArea').empty().append(myControl.renderFunctions.createTemplateInstance('productListTemplate',"productViewer"));
...
//				myControl.util.dump(" -> subcats present");
				numRequests += myControl.ext.store_navcats.util.getSubsFromDetail(catSafeID,'appCategoryDetailMax');
				}
			}
...
//get the first few product. The rest will be retrieved later in the process, but this lets us get as much in front of the user as quickly as possible
				numRequests += myControl.ext.store_prodlist.util.getProductDataForLaterUse(myControl.data['appCategoryDetail|'+catSafeID]['@products'],'mutable');
				}
			}
