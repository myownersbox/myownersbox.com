...
//!!! hhhmm.. needs fixin. need to compensate 'i' for hidden categories.
			...
			view360inModal : function($tag,data){
				...
				$tag.removeClass('displayNone').addClass('pointer'); //shows img, which is hidden by default to prevent a flicker
				$tag.click(function(){
					$("<div>").attr('title','360 viewer').append("<div class='center'><iframe src='"+data.bindData.cleanValue+"' width='600' height='620' \/><\/div>").dialog({modal:true, width:"90%", height:700});
					})
...
//handle cases where the homepage is treated like a category page. happens in breadcrumb.
				...
				myControl.util.dump("GOT HERE ! -> 1.2");
//			$('title').text("MyOwnersBox.com"+relativePath); // breaks IE 8.
