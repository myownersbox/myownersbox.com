...
//				myControl.util.dump('$.param( P )'+$.param(P)); // need the pushstate to support params.
...
setTimeout("myControl.ext.myownersbox_configurator.util.makeDrawersDraggable()",2500);
				},
...
//if no pid is passed, just empty the existing spots
				if(pid)	{
					var $dragContainer = $("<span class='draggable'>").attr('data-pid',pid).html(myControl.util.makeImage({"h":"129","w":"129","b":"tttttt","name":myControl.data['appProductGet|'+pid]['%attribs']['zoovy:prod_image2'],"tag":1}));
...
//				myControl.util.dump(" -> URL = "+url);
			...
			buildURIVars : function()	{
				...
				var s1 = myControl.util.isSet($('#storageContainerCategories .selected').attr('data-catsafeid')) ? $('#storageContainerCategories .selected').attr('data-catsafeid') : ".drawers.major-league-baseball";
