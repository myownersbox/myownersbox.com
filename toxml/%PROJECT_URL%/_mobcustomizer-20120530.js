...
//				myControl.util.dump("BEGIN mobcustomizer.util.containerSizeSelected. uriParams to follow");
//				myControl.util.dump(myControl.ext.myownersbox_configurator.vars.uriParams);
...
//used when the page is printed.
var imageHTML = myControl.util.makeImage({"h":"500","w":"500","b":"ffffff","name":myControl.data['appProductGet|'+pid]['%attribs']['zoovy:prod_image1'],"tag":1});
imageHTML += myControl.util.makeImage({"h":"500","w":"500","b":"ffffff","name":myControl.data['appProductGet|'+pid]['%attribs']['zoovy:prod_image2'],"tag":1})
imageHTML += myControl.util.makeImage({"h":"500","w":"500","b":"ffffff","name":myControl.data['appProductGet|'+pid]['%attribs']['zoovy:prod_image3'],"tag":1})
$('#organizerPreviewForPrint').empty().html(imageHTML);
...
//otherwise, they 'stay' in the totals and product list, et all.
							myControl.ext.myownersbox_configurator.util.drawerAssignedToSpot(i);  // ### !!! THIS IS THE LINE CAUSING URI PARAMS TO BLANK OUT
...
//if no pid is passed, just empty the existing spots
				if(pid)	{
					var $dragContainer = $("<span class='draggable'>").attr('data-pid',pid).html(myControl.util.makeImage({"h":"131","w":"131","b":"ffffff","name":myControl.data['appProductGet|'+pid]['%attribs']['zoovy:prod_image2'],"tag":1}));
