...
//default to showing the homepage if page info is blank. that would mean giveMeRelevantInfoToPage retrned nothing useful.
					if(!myControl.util.isSet(pageInfo) || pageInfo.indexOf('index') >= 0)	{
						myControl.ext.myownersboxRIA.util.showPage('.');
						}
					else	{
...
//on initial load, P will be blank.
				...
				myControl.util.dump("POPSTATE Executed.  pageType = "+pageType+" and pageInfo = "+pageInfo);
...
//				myControl.util.dump(" last PushState:");
				myControl.util.dump(window.history);
				myControl.util.dump(window.history[-1]);
...
myControl.util.dump("WARNING - unknown nav type ["+newNav+"] specified for changeNavTo");
				...
				myControl.ext.myownersboxRIA.util.bindNav();
				if(article)
...
//figures which nav tree is visible and returns the id. probably don't need this for this RIA.
			...
			bindNav : function()	{
myControl.util.dump(" BEGIN bindNav");
...
//handles inline validation
			...
			handleAddToCart : function(formID)	{
				tagObj = {'callback':'itemAddedToCart','extension':'store_product'};
				myControl.ext.store_product.calls.addToCart.init(formID,tagObj);
