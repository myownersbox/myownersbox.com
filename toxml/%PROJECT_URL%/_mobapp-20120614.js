...
// it'll only be there on the computer it was just placed from. Fetch it. Check the timestamp to make sure it's recent.
				if(P && P.uriParams && P.uriParams.orderid)	{
					var orderID = P.uriParams.orderid
					var dp = 'order|'+orderID;
					myControl.util.dump(" -> P and P.orderid ["+orderID+"] are set");
					myControl.model.fetchData(dp);
					if(!$.isEmptyObject(myControl.data[dp]))	{
						var age = myControl.util.unixNow() - myControl.data[dp].ts;
						myControl.util.dump(" -> age: "+age);
						if(age > (60*60*4))	{
							myControl.util.dump(" -> the ts on this order is old. require login to view.");
