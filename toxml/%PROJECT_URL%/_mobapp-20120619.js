...
//if this is a custom extension and you are loading system extensions (prodlist, etc), then load ALL templates you'll need here.
		"templates" : ['productListTemplate','mpControlSpec','breadcrumbTemplate','categoryTemplate','productTemplate','cartViewer','productListTemplateCart','prodReviewSummaryTemplate','productChildrenTemplate','prodReviewsTemplate','reviewFrmTemplate','subscribeFormTemplate','categoryListTemplate','orderLineItemTemplate','orderContentsTemplate','cartSummaryTemplate','orderProductLineItemTemplate','companyTemplate','customerTemplate','productDetailedTemplate','homepageTemplate','configuratorTemplate','leftColRootCatsTemplate','searchTemplate','faqTopicTemplate','faqQnATemplate','billAddressTemplate','shipAddressTemplate'],
		"user" : {
			"recentSearches" : [],
			"recentlyViewedItems" : []
			},
		"dependAttempts" : 0,  //used to count how many times loading the dependencies has been attempted.
		"dependencies" : ['store_prodlist','store_navcats','store_product','convertSessionToOrder','store_search','store_cart','store_crm','myownersbox_configurator'] //a list of other extensions (just the namespace) that are required for this one to load
		},
	...
	calls : {
		buyerOrderGet : {
			...
			dispatch : function(obj,tagObj,Q)	{
				...
				obj['softauth'] = "order";
//				obj['cartid'] = "ngN43u9N0Y3Wk8C5Diwddb8RJ";
				obj["_tag"] = tagObj;
...
//you may or may not need it.
		...
		startMyProgram : {
			onSuccess : function()	{
				...
				var uriParams = myControl.util.getParametersAsObject(document.location.href.split('?')[1]); // do this first before a anything else occurs, so params are preserved.
//				myControl.util.dump("uriParams follow:");
//				myControl.util.dump(uriParams);
//attach an event to the window that will execute code on 'back' some history has been added to the history.
				...
				if(uriParams.debug == 1)	{
					$('.debug').show().append("V: "+myControl.vars.release);
					}
...
//get some info to have handy for when needed (cart, profile, etc)
				...
				myControl.ext.myRIA.util.handleAppInit(uriParams); //checks url and will load appropriate page content.
...
//adds submit functionality to search form. keeps dom clean to do it here.
				...
				setTimeout("runAfterAppInit()",3500); //contains all third party code.
				},
			onError : function(d)	{
...
//will be set to 'company'
				if(myControl.vars.waitingOnProfile)	{
//					myControl.util.dump(" !!!!!!!!!!!! GOT HERE !!!!!!!!!!!!");
					showContent('company',myControl.vars.waitingOnProfile);
					myControl.vars.waitingOnProfile = null;
					}
...
*/					
				...
				myControl.model.dispatchThis();
				if(typeof tmp['%page'] == 'object' && (myControl.util.isSet(tmp['%page']['picture5']) || myControl.util.isSet(tmp['%page']['picture1'])))	{
...
//successful login.	
				...
				$('#loginSuccessContainer').prepend("Thank you, you are now logged in. <br />");
				$('#modalLoginForm').hide();
				$('#recoverPasswordContainer').hide();
...
//returns # of requests needed. so if 0 is returned, no need to dispatch.
					myControl.ext.store_prodlist.util.buildProductList({"templateID":"productListTemplate","withInventory":1,"withVariations":1,"parentID":"resultsProductListContainer","items_per_page":15,"csv":myControl.data[tagObj.datapointer]['@products']})
					}
...
//!!! hhhmm.. needs fixin. need to compensate 'i' for hidden categories.
			...
			addLinkOnclick : function($tag,data)	{
				...
				$tag.removeClass('displayNone').addClass('pointer').click(function(){
					window.open(data.bindData.cleanValue,'_blank');
					});
			...
			view360inModal : function($tag,data)	{
				...
				$tag.click(function(){
					...
					myControl.ext.myRIA.action.show360Viewer(data.bindData.cleanValue);
					});
...
//					myControl.util.dump("GOT HERE");
				...
				case 'checkout':
					infoObj.back = 0; //always skip adding a pushstate in checkout.
...
//assumes the faq are already in memory.
			showFAQbyTopic : function(topicID)	{
				...
				var $target = $('#faqDetails4Topic_'+topicID).empty().toggle();
				if(!topicID)	{
					...
					$target.append("It appears there are no faq for this topic.");
					myControl.util.dump(" -> No data is present");
					}
				else	{
					if($target.length > 1)	{
//contents have already been rendered
						}
					else	{
...
//otherwise, loads page content based on legacy linking syntax. (if url is /product/pid, then pid gets loaded)
			handleAppInit : function(uriParams)	{
...
//				myControl.util.dump(uriParams);
				...
				if(location.protocol == 'file:')	{
					myControl.util.dump("Working locally? got registerd as a file: protocol");
					pageInfo.pageType = 'homepage';
					pageInfo.navcat = '.';
					}
				else	{
// will return either the safe path or pid or something else useful
					pageInfo = this.detectRelevantInfoToPage(window.location.href); // ## SANITY - this will blank out anything previously set in pageInfo
//					pageInfo.back = 0; //skip adding a pushState on initial page load.
					pageInfo.isInit = 1; //skip adding a pushState on initial page load.
					}

				pageInfo.uriParams = uriParams;
				myControl.ext.myRIA.action.showContent('',pageInfo);
				return pageInfo //returning this saves some additional looking up in the appInit
				},

//function is in store_crm in 201225+
...
//					myControl.util.dump(" after strip trailing slash, r = "+r.navcat);
					...
					r.pageType = 'customer'
					r.show = url.split('/customer/')[1]; //left with order_summary or order_summary/ or order or order/status
					if(r.show.charAt(r.show.length-1) == '/')	{r.show = r.show.slice(0, -1)} //strip trailing /
					if(r.show.indexOf('/') > -1){r.show = r.show.split('/')[0]} //handle some legacy url syntax (like /customer/order/status
					}
				else if(url.indexOf('/company/') > 0)	{
					r.pageType = 'company'
					r.show = url.split('/company/')[1]; //left with order_summary or order_summary/ or order or order/status
					if(r.show.charAt(r.show.length-1) == '/')	{r.show = r.show.slice(0, -1)} //strip trailing /
					if(r.show.indexOf('/') > -1){r.show = r.show.split('/')[0]} //handle some legacy url syntax (like /customer/order/status
					}
...
//					if(r.show.indexOf('?') > 0){r.show = r.show.split('?')[1]} //remove any uri vars. commented out 2012-05-11. uri vars handled above.
						...
						myControl.util.dump(" -> r.show: "+r.show);
						if(r.show.indexOf('/') > -1){r.show = r.show.split('/')[0]} //handle some legacy url syntax (play friendly with /contact/)
						myControl.util.dump(" -> r.show: "+r.show);
						}
					}
...
//rather than having all the params in the dom, just call this function. makes updating easier too.
	...
	//add item to recently viewed list IF it is not already in the list.				
			...
			changeNavTo : function(P)	{
				myControl.util.dump("BEGIN myRIA.util.changeNavTo ("+P.pageType+")");
//				myControl.model.abortQ('mutable');  // ### NOTE - test this when DEV is stable.
				...
				myControl.util.dump(" -> newNav: "+newNav);
				myControl.util.dump(" -> article: "+article);
...
//new nav is 'customer' or 'company'. search nav may or may not be supported for this RIA
				if(!newNav)	{myControl.util.dump("WARNING - nav type not specified for changeNavTo");}
					...
					$('#mainContentArea').empty().append(myControl.renderFunctions.transmogrify('mainContentArea_'+newNav,newNav+'Template',myControl.data['appProfileInfo|DEFAULT']))
					}
				else if(newNav == 'customer')	{
					$('#mainContentArea').empty().append(myControl.renderFunctions.createTemplateInstance(newNav+'Template','mainContentArea_'+newNav))
					}
				else if (newNav == 'company')	{
//bit of a cheap hack. profile data may not be in memory by this point. the profile callback checks this var and will execute the transmogrify.
//I recognize this isn't a good way to handle this and there is a bigger issue. That'll be resolved in the next version of the RIA.
					if(typeof myControl.data['appProfileInfo|DEFAULT'] != 'object')	{
						myControl.vars.waitingOnProfile = P;
						}
					else	{
						$('#mainContentArea').empty().append(myControl.renderFunctions.transmogrify('mainContentArea_'+newNav,'companyTemplate',myControl.data['appProfileInfo|DEFAULT']))
						}
					}
				else	{
//unknown nav.
...
//make sure form is showing and previous messaging is removed/reset.
				...
				$('#recoverPasswordContainer').show(); //contains actual form and password recovery form (second id)
...
//make sure they're available.
			showArticle : function(subject,P)	{
//				myControl.util.dump("BEGIN myRIA.util.showArticle 1.4 ("+subject+")");
//				myControl.util.dump(" -> P:");
//				myControl.util.dump(P);
//oops. some legacy urls that didn't quite match.
				if(subject == 'order')	{
//					myControl.util.dump(" -> subject: "+subject);
					subject = 'orders';
//					myControl.util.dump(" -> subject: "+subject);
					}
				$('#mainContentArea article').hide(); //hide all the articles. later, the one desired will be toggled on.
//				$('#rightCol [href=#'+subject+']').addClass('ui-state-highlight'); //make page in focus link stand out.
				$('#globalMessaging').empty();

				var authState = myControl.sharedCheckoutUtilities.determineAuthentication();
//				myControl.util.dump(" -> authState = "+authState);
				
//				myControl.util.dump(" -> URI params are dealt with.");
				
				if(P && P.uriParams && P.uriParams.orderid && P.uriParams.cartid)	{
					...
					$invoice.append(myControl.renderFunctions.createTemplateInstance('orderContentsTemplate','orderContentsTable_'+myControl.util.makeSafeHTMLId(orderID)))
//					myControl.util.dump($invoice);
					$invoice.appendTo($('#customerMainContentArea'));
					myControl.ext.myRIA.calls.buyerOrderGet.init({'orderid':orderID,'cartid':cartID},{'callback':'showOrder','extension':'store_crm','templateID':'orderContentsTemplate','parentID':'orderContentsTable_'+myControl.util.makeSafeHTMLId(orderID)},'mutable');
...
//don't show any pages that require login unless the user is logged in.
				else if((authState != 'authenticated') && (subject == 'orders' || subject == 'wishlist' || subject == 'changepassword' || subject == 'forgetme' || subject == 'myaccount'))	{
...
//this will be fluid, as it'll take them where they expected to go.
					...
					myControl.util.dump(" -> not dealing w/ order and login not required. show: "+subject);
...
//	myControl.util.dump("myRIA.util.showPage. catsafeid == .customizer");
	myControl.ext.myownersbox_configurator.actions.initConfigurator(P.uriParams);
	}
else	{
	if(catSafeID == '.')	{
...
//was having NAN issues with numSpots - numDrawers, so used toFixed to be sure vars were treated as integers.
							$target.append("Add "+(numSpots.toFixed(0) - numDrawers.toFixed(0))+" more drawers to receive 15% off your storage organizer");
							}
						}
					}
				return hasContainer;
