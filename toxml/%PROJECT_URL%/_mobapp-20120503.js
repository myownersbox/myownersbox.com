...
//get some info to have handy for when needed (cart, profile, etc)
				myControl.calls.appProfileInfo.init(myControl.vars.profile,{"callback":"haveProfileData","extension":"myownersboxRIA"});
				myControl.ext.store_cart.calls.cartItemsList.init({'callback':'updateMCLineItems','extension':'myownersboxRIA'},'mutable');
...
/* have top level category page info at the ready... */
				myControl.ext.store_navcats.calls.appPageGet.init({'PATH':'.drawers','@get':['picture1','picture2','picture3','picture4','description1','wide_banner1','header_subcats1','header1','header2']});
				
				myControl.ext.store_navcats.calls.appPageGet.init({'PATH':'.stackits','@get':['picture1','picture2','picture3','picture4','description1','wide_banner1','header_subcats1','header1','header2']});
				
				myControl.ext.store_navcats.calls.appPageGet.init({'PATH':'.complete_systems','@get':['picture1','picture2','picture3','picture4','description1','wide_banner1','header_subcats1','header1','header2']});
...
//add root categories to left column.
				myControl.ext.store_navcats.util.getChildDataOf('.',{'parentID':'rootcategoryListContainer','callback':'addCatToDom','templateID':'leftColRootCatsTemplate','extension':'store_navcats'},'appCategoryDetailMore');
				if(typeof myControl.data['appCategoryDetail|'+catSafeID]['@subcategoryDetail'] == 'object')	{
					myControl.ext.store_navcats.util.getChildDataOf(catSafeID,{'parentID':'subcategoryListContainer','callback':'addCatToDom','templateID':'categoryThumbTemplate','extension':'store_navcats'},'appCategoryDetailMax');
					}
				else	{
...
//!!! hhhmm.. needs fixin. need to compensate 'i' for hidden categories.
					for(var i = 0; i < size; i +=1)	{
						if(myControl.data['appCategoryDetail|'+data.bindData.cleanValue]['@subcategoryDetail'][i].pretty[0] != '!')	{
							...
							o += "<li onClick=\"myControl.ext.myownersboxRIA.util.handlePageContent('category','"+catSafeID+"');\"> &#187; "+myControl.data['appCategoryDetail|'+data.bindData.cleanValue]['@subcategoryDetail'][i].pretty;
...
//				myControl.util.dump("BEGIN handlePageContent. type: "+pageType+" and info: "+pageInfo+" and skipPushState: "+skipPushState);
				...
				case 'category':
					if(pageInfo == '.customizer')	{
						...
						skipPushState = true; //configurator will add it's own pushState
						}
					else	{
...
// if we redirect to ssl for checkout, it's a new url and a pushstate isn't needed, so a param is added to the url.
						document.location = myControl.vars.secureURL+"c="+myControl.sessionId+"/checkout.cgis?SENDER=JCHECKOUT&SKIPPUSHSTATE=1";
...
// this condition is met after the redirect (such as paypalEC), so skippushstate is checked on URI.
				...
				case 'cart':
						myControl.ext.myownersboxRIA.util.showPage('.'); //load homepage content in BG.
						myControl.ext.myownersboxRIA.util.showCart();
						break;
...
//				myControl.util.jumpToAnchor('#top'); //this will totally F up the push/pop state feature.
				...
				if(skipPushState == true)	{
					...
					//skipped when executed from a 'pop' (back) or when initial page loads.
					}
				else	{
...
//GA
// 				myControl.util.dump("_trackPageview : "+pageType+'/'+pageInfo);
//				_gaq.push(['_trackPageview',pageType+'/'+ pageInfo]);
...
/* URL handing functions (push and pop state) */
...
//http://spoiledmilk.dk/blog/html5-changing-the-browser-url-without-refreshing-page
			...
			addPushState : function(P)	{
				...
				var relativePath;
				myControl.util.dump("BEGIN addPushState. ["+P.pageType+"/"+P.pageInfo+"]");
...
//				myControl.util.dump(" -> adding push state for "+relativePath);
				try	{
...
//nuke existing content and error messages.
				...
				myControl.ext.store_product.calls.appReviewsList.init(pid);
				myControl.ext.store_product.calls.appProductGet.init(pid,{'callback':'showProd','extension':'myownersboxRIA','parentID':'productViewer'});
				myControl.model.dispatchThis();
...
//### need to update this to check if data is present. may not be if user came straight to a cat page.
						$bc.append(myControl.renderFunctions.transmogrify({"id":"breadcrumb_"+s,"catsafeid":s},'categoryTemplate',myControl.data['appCategoryDetail|'+s]))
...
//get category data for displaying page
					myControl.ext.store_navcats.calls.appCategoryDetailMax.init(catSafeID,{"callback":"showPageContent","extension":"myownersboxRIA"});
...
//handles inline validation
			loginFrmSubmit : function(email,password)	{
				...
				if(errors == ''){
					...
					myControl.ext.store_cart.calls.cartItemsList.init('','immutable'); //cart needs to be updated as part of authentication process.
...
//assumes the faq are already in memory.
			...
			handleAddToCart : function(formID)	{
				myControl.ext.store_product.calls.cartItemsAdd.init(formID,{'callback':'itemAddedToCart','extension':'store_product'});
				myControl.calls.refreshCart.init({'callback':'updateMCLineItems','extension':'myownersboxRIA'},'immutable');
				myControl.model.dispatchThis('immutable');
				},
