...
//				for(var i = 0; i < myControl.data['categoryDetail|.storage-containers'].subcategoryCount; i +=1)	{
...
//					myControl.util.dump(" -> spot "+($(this).index()+1)+" data-pid: "+$(this).attr('data-pid'));
					if($(this).attr('data-pid')) {total += (myControl.data['getProduct|'+$(this).attr('data-pid')]['%attribs']['zoovy:base_price'] * 1)}
					});
				if(total)	{
					$('#configuratorSubTotal').empty().text("subtotal: "+myControl.util.formatMoney(total,'$',2,true));
					}
				else	{
					$('#configuratorSubTotal').empty(); //hide at any zero. necessary when items are added and all are removed.
					}
				},
...
//if no pid is passed, just empty the existing spots
				if(pid)	{
					var $dragContainer = $("<span class='draggable'>").attr('data-pid',pid).html(myControl.util.makeImage({"h":"129","w":"129","bg":"ffffff","name":myControl.data['getProduct|'+pid]['%attribs']['zoovy:prod_image1'],"tag":1}));
...
//					myControl.util.dump(" => typeof $spot.attr('data-pid') = "+typeof $spot.attr('data-pid'));
					...
					name = myControl.data['getProduct|'+pid]['%attribs']['user:prod_organization'] ? myControl.data['getProduct|'+pid]['%attribs']['user:prod_organization'] : myControl.data['getProduct|'+pid]['%attribs']['zoovy:prod_name'];
					cssClass = 'occupied';
					price = myControl.util.formatMoney(myControl.data['getProduct|'+pid]['%attribs']['zoovy:base_price'],'$',2,true)
...
//add it to the obj if pid isn't already in there, otherwise increment existing value. same drawer may be in several spots.
				...
				if($.isEmptyObject(obj))	{
					...
					myControl.ext.store_cart.calls.getCartContents.init({'callback':'displayCart','extension':'store_cart','parentID':'modalCartContents'},'immutable');
...
//		myControl.util.dump(" -> ui.attr('data-pid') : "+ui.draggable.attr('data-pid'));
		...
		$('#selectedDrawerContainer').find('.dropItHere').removeClass('dropItHere');
        }
    });
